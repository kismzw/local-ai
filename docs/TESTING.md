...
4. Run `./tests/tool-isolation.sh`, `./tests/tool-bridge-allowlist.sh`, and `./tests/tool-bridge-codex.sh`; they prove host-path isolation, explicit `/workspace` and `/data` mounts, secret-path masking, authenticated writable commands, output capping, and contained Git behavior. Record whether unprivileged network namespaces work.
