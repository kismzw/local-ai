...
4. Run `./tests/tool-isolation.sh`, `./tests/tool-bridge-allowlist.sh`, and `./tests/tool-bridge-codex.sh`; they prove host-path isolation, a read-only `/workspace` mount with an actual failed write, optional `/data:ro` with an actual failed write, secret-path masking, authenticated writable commands, output capping, and contained Git behavior. Record whether unprivileged network namespaces work.
