...
- Inference SIF unhealthy: verify filenames in `.env`, `images/`, `models/`, and `./scripts/logs.sh llama-server`.
- Out of memory: return `MODEL_CONTEXT_SIZE` to 32768, keep `MODEL_PARALLELISM=1`, and leave KV cache at `q8_0`.
