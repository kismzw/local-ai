...
def test_missing_receipt_adopts_and_then_verifies(tmp_path: Path):
    image = tmp_path / "image.sif"
    image.write_bytes(b"first")
    receipt = tmp_path / "receipts.toml"
    source = "docker://repo@sha256:" + "a" * 64
    assert image_receipts.verify_or_adopt(receipt, "llama", source, image, None) == "adopted"
    assert image_receipts.verify_or_adopt(receipt, "llama", source, image, None) == "verified"
...
def test_receipt_rejects_source_or_artifact_tampering(tmp_path: Path):
    ...
    with pytest.raises(image_receipts.ReceiptError, match="source"):
        image_receipts.verify_or_adopt(receipt, "llama", "docker://repo@sha256:" + "b" * 64, image, None)
    image.write_bytes(b"changed")
    with pytest.raises(image_receipts.ReceiptError, match="artifact_sha256"):
        image_receipts.verify_or_adopt(receipt, "llama", source, image, None)
...
def test_refresh_accepts_new_artifact_for_same_source(tmp_path: Path):
    ...
    receipt = tmp_path / "receipts.toml"
    source = "docker://python@sha256:" + "a" * 64
    image_receipts.record(receipt, "tool", source, image, definition)
    image.write_bytes(b"rebuilt differently")
    image_receipts.record(receipt, "tool", source, image, definition)
    assert image_receipts.verify_or_adopt(receipt, "tool", source, image, definition) == "verified"
...
def test_tool_definition_change_is_rejected(tmp_path: Path):
    ...
    with pytest.raises(image_receipts.ReceiptError, match="definition_sha256"):
        image_receipts.verify_or_adopt(receipt, "tool", source, image, definition)
